...
async function main() {
  ...
  const locale = getArg("--locale") || "zh-CN";
  const translationGroupId = getArg("--translation-group-id");

  if (!file) { console.error("用法: node publish.js --file \"文章.md\" [--cover \"封面.png\"] [--locale \"en-US\"] [--translation-group-id \"ID\"] [--no-publish] [--seo-title \"...\"] [--seo-description \"...\"] [--summary \"...\"]"); process.exit(1); }
  ...
  const articleData = {
    title: article.title,
    slug: customSlug || article.slug,
    content: article.content,
    summary: summary || article.summary,
    locale,
  };
  if (translationGroupId) articleData.translationGroupId = translationGroupId;
  ...
  console.log(`  语言: ${articleData.locale}`);
  if (articleData.translationGroupId) console.log(`  翻译组: ${articleData.translationGroupId}`);
